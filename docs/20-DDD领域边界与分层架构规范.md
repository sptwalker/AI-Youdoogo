...
> 状态：P0～P4 与兼容层退出验证已完成（2026-07-28）。
...
| 文档与全量验证 | 已完成 | Ruff、Mypy（702 个源文件）、932 个非交付 Pytest、前端 test/lint/build、`040_workflow_run_engine` 单一 head、app/worker smoke 与架构门禁测试（boundaries/runtime/governance）均通过；兼容层退出后的相关测试和 Graphify 代码架构复核持续作为提交门禁 |

迁移期 `app/api/core/models/schemas/agents/llm/integrations` 中尚有历史入口；`app/services` 与
`app/knowledge` 已整体退出。新实现必须进入对应 Context 或 Platform，禁止恢复横向 facade。仓库内静态
调用证据与不得回归规则见 11.3～11.5。本地保留的兼容层退出历史 Graphify 快照为
10,646 个节点、23,863 条边；变基后的依赖事实以当前 AST import graph 和架构门禁为准，历史图健康检查
无悬空端点、缺失端点、自环或折叠边。按本次交付要求，17 个演示 SVG 保留为待选语义更新，不影响代码依赖、
写入所有权和质量门验收。
...
`app/services` 与 `app/knowledge` 已整体退出。历史职责的正式归属如下，后续调用只能进入这些边界：

| 历史职责 | 正式归属 | 当前入口 |
|---|---|---|
| 配置、Provider 卡片、连通性 | System Configuration / AI Provider Management / Connectivity | 各 Context 的 `public` 或 `entrypoints` |
| 身份、飞书登录、组织同步、专家角色、授权 | Identity / Organization Structure / Expert Management / Access Control | 各 Context published operations |
| Wiki、索引、embedding、检索、语义、记忆 | Foundations / Knowledge 各叶子 Context | `knowledge_indexing`、`knowledge_retrieval`、`organizational_memory` 等正式边界 |
| 对象上传、下载与对象名 | Platform Object Storage | `app.platform.object_storage` |
| Planning、durable workflow、恢复、投影、事件处理 | Work Planning / Workflow Runtime / Bootstrap | Context contracts/entrypoints/infrastructure 与 `app.bootstrap.workflow_*` |
| TaskCard 状态和 Workflow→Task 投影 | Task Management | domain state machine、published operations 与 SQLAlchemy adapter |
| 查询、交付、协作 Agent capability | Governed Data Query / Deliverable Management / Collaboration Requests | 各 Context 的 `entrypoints.agent_capability` |
| 群消息、归档、实时通知 | Group Messaging / Platform Realtime | Context operations/event contract 与 Platform transport |
| 审计、评估、反馈、反思 | Audit Trail / AI Quality | published commands、queries 与 application ports |
| Proposal、Meeting、Assistant、Analytics | 对应 Business Context | Context public/entrypoints |
| Outbox repository、租约与重试 | Platform Outbox + Workflow Runtime worker | `app.platform.outbox` 与 Runtime worker |
| 飞书等供应商协议 | `app/integrations/*` | 聚焦 client capability；生命周期由 Bootstrap 关闭 |

`app/models` 仍是迁移期 ORM 位置，不得因此重新建立横向 Service。`app/agents` 中保留的 runtime adapter
也必须依赖 Context contracts/capabilities，而不是恢复已删除的 facade。

### 11.3 已退出的 Legacy Facade

2026-07-28 的清理已删除以下横向兼容面：

- `app/services/**`：配置、身份、组织、知识、协作、任务、Workflow、治理、Outbox、Realtime 等全部 facade；
- `app/knowledge/**`：embedding、ingest、storage 与包级兼容入口；
- `app/agents/scheduler.py` 与 `app/agents/workflow_engine.py`：旧调度桥已由正式 Planning/Runtime 路径替代。

仓库不再提供这些 Python import 路径。测试中的路径字符串只允许用于构造历史依赖图或断言“旧路径不得
回归”，不能作为 importlib、monkeypatch、插件配置或运行时入口。

仍处于迁移期的横向目录是 `app/api`、`app/core`、`app/models`、`app/schemas`、`app/agents`、`app/llm`
与 `app/integrations`；它们不属于本轮已删除 facade，也不得反向依赖不存在的 `app.services`/`app.knowledge`。

### 11.4 静态调用分类与零调用证据

扫描范围覆盖 `app/`、`tests/`、`scripts/` 与 migration/tooling。当前结果是：

1. `app/services` 和 `app/knowledge` 已不再是包或目录，旧 `scheduler.py`、`workflow_engine.py` 也已删除；
2. `app/`、`tests/`、`scripts/`、`alembic/` 中不存在对上述旧路径的真实 Python import；
3. Durable workflow 测试通过 `tests/workflow_testkit.py` 组装正式 Planning/Runtime/Task 端口，不再借用 Service；
4. 群归档测试从 Group Messaging event contract 取得 `DISBAND_ARCHIVE_EVENT` 与 `archive_file_id`，并通过
   Bootstrap `archive_channel` seam 注入归档行为；
5. Skill 测试直接使用 Data Query、Deliverable、Collaboration capability adapter；
6. Bootstrap lifecycle 直接使用 System Configuration、AI Provider、Identity 和 Feishu Integration 正式边界；
7. 架构测试把旧模块名保留为负向 fixture/forbidden prefix，并显式检查已退出目录与旧 Agent bridge 不得回归；
   兼容退出后的全量 Pytest 收集基线为 729 项。

静态文本搜索仍会命中上述负向断言和人为构造的历史 import graph；判断“真实调用”时必须区分 Python AST
import 与测试数据字符串，不能为了获得文本零命中而删除架构门禁。

### 11.5 Facade 退出条件与后续迁移指导

本轮已满足删除条件：真实调用迁入正式边界、独占行为与注入缝已下沉、特征测试继续覆盖事务/幂等/权限/
归档/重试语义，并建立旧目录不得回归的门禁。此次删除应按仓库级 breaking change 处理；若仓外插件仍引用
旧路径，必须迁移插件，禁止在仓内恢复兼容 facade。

后续规则：

1. 新代码禁止创建 `app/services`、`app/knowledge`，也禁止建立同义的 `service_facades` 横向目录；
2. 测试必须从正式 Context/Platform 路径 import；需要跨 Context 组装时使用测试夹具或 Bootstrap seam；
3. 负向架构测试持续检查生产代码、脚本和测试代码没有真实旧路径 import；
4. 后续清理 `app/api/core/models/schemas/agents/llm/integrations` 时继续逐个迁移所有权，不把本轮清理扩大为重写；
5. 每次删除剩余兼容入口后运行 Ruff、Mypy、相关 Pytest、全量收集与 import graph 检查，再刷新 Graphify。
