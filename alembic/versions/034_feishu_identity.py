...
- sys_user.feishu_open_id/en_name/title/mobile/avatar_url（同步+SSO 身份）
- 映射键部分唯一索引（软删可重建，与现有范式一致）。纯加列，历史行取默认。
"""
...
down_revision: str | None = "033_eval_case"
...
def upgrade() -> None:
    ...
    op.add_column("sys_user", sa.Column("avatar_url", sa.String(512), nullable=False,
                                        server_default=""))
    op.create_index(
        "uq_user_feishu", "sys_user", ["feishu_open_id"],
        unique=True, postgresql_where=sa.text(_ACTIVE),
    )


def downgrade() -> None:
    op.drop_index("uq_user_feishu", table_name="sys_user")
