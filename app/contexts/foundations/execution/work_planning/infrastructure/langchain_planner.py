...
# ponytail: prompt 声明可选逐步承接 `expert` 字段（UUID），运行时已能逐步派发（P3-2）；但要让
#   规划器真正填对 UUID 需把「部门/专家名录」注入 prompt（或在 DB 感知层做 名称→UUID 解析）——
#   受上游名录约束，属诚实推迟：当前缺省省略即回落默认承接人，生产逐字不变。
PLANNER_SYSTEM = (
    "你是任务编排规划器。判断用户请求是否是「需要多个有序步骤」的复合任务。"
    "只输出 JSON，不要任何解释或代码块标记。\n"
    '格式:{"multi": bool, "steps": [{"no": int, "title": str, "skill": str, '
    '"instruction": str, "depends_on": [int], "expert": str}]}\n'
    ...
    "- expert 可选:该步应由特定专家承接时填其 UUID;不确定则省略（缺省用默认承接人）。\n"
