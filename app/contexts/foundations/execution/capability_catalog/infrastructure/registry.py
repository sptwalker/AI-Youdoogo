...
CAPABILITY_DEFINITIONS = (
    ...
    CapabilityDefinition(
        key="create_operational_proposal",
        version="1.0",
        label="生成运营提案",
        description="就运营议题产出结构化优化提案（内部顾问输出·仅供参考·不构成决策·不对外发送）",
        input_schema_json='{"type":"object","required":["topic"]}',
        output_schema_json='{"type":"object"}',
        risk=CapabilityRisk.LOW,
        side_effect=CapabilitySideEffect.INTERNAL_WRITE,
        permission_keys=("create_operational_proposal",),
        handler_identity="operational_analytics.proposal",
        feature_flag="agent_create_operational_proposal",
    ),
    CapabilityDefinition(
        key="generate_minutes",
        version="1.0",
        label="生成会议纪要",
        description="读上游会议为已建会议机械生成纪要（机械步·内部写·仅验收后自动执行、不可见）",
        input_schema_json='{"type":"object"}',
        output_schema_json='{"type":"object"}',
        risk=CapabilityRisk.LOW,
        side_effect=CapabilitySideEffect.INTERNAL_WRITE,
        permission_keys=("convene_consultation",),
        handler_identity="meeting_management.generate_minutes",
        feature_flag="agent_convene_consultation",
        default_enabled=False,
    ),
    CapabilityDefinition(
