...
CAPABILITY_DEFINITIONS = (
    ...
    CapabilityDefinition(
        key="convene_consultation",
        version="1.0",
        label="整理紧急会商草稿",
        description="把研判议题整理成紧急会商草稿（红线·先出草稿待真人验收后才建会并定向通知，本步不建会不通知）",
        input_schema_json='{"type":"object"}',
        output_schema_json='{"type":"object"}',
        risk=CapabilityRisk.MEDIUM,
        side_effect=CapabilitySideEffect.INTERNAL_WRITE,
        permission_keys=("convene_consultation",),
        handler_identity="convene_consultation.compose",
        feature_flag="agent_convene_consultation",
        default_enabled=False,
    ),
    CapabilityDefinition(
        key="send_email",
        version="1.0",
        label="整理邮件草稿",
        description="把已确认的内容整理成邮件草稿（红线·外部写·先出草稿待真人验收后才发送，本步不发信）",
        input_schema_json='{"type":"object"}',
        output_schema_json='{"type":"object"}',
        risk=CapabilityRisk.HIGH,
        side_effect=CapabilitySideEffect.EXTERNAL_WRITE,
        permission_keys=("send_email",),
        handler_identity="send_email.compose",
        feature_flag="agent_send_email",
        default_enabled=False,
    ),
    CapabilityDefinition(
        key="knowledge_index",
