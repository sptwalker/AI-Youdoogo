"""Stable Usage & Budget governance facade for cross-context callers.

LLM usage 记账/预算判定的正典入口。消费方（agent 执行、知识问答、蒸馏、评分……）
一律经此 public 记账，不再直连 ``app.llm.usage`` 兼容层。``extract_usage`` 走鸭子类型
（只读 ``usage_metadata``），不引 langchain，故本模块对完成语义零依赖。
"""
