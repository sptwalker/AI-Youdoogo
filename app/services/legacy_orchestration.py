"""Legacy synchronous TaskCard-only DAG adapter kept for migration compatibility."""
...
from app.services.workflow_planning import MAX_STEPS
...
async def step_cards(db: AsyncSession, parent_id: uuid.UUID) -> list[TaskCard]:
    steps = await task_service.list_tasks(db, parent_id=parent_id, limit=MAX_STEPS + 2)
...
async def run_step(
    ...
    for consulted, consult_record in result.consult_replies:
        text += (
            f"\n\n---\n【{consulted.name} 答复】\n"
            f"{consult_record.output_content or consult_record.error_msg or ''}"
        )
    text = skills.fold_notes(text, result)
    await to_reported(db, step, role.id, f"执行 status={record.status}", text)
    return None if record.status == "failed" else result
