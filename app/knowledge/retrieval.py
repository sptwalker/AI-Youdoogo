...
async def search(db: AsyncSession, query: str, top_k: int = 5) -> list[Hit]:
...
async def answer(
    db: AsyncSession, query: str, top_k: int = 5, *, user_id: uuid.UUID | None = None
) -> dict[str, Any]:
    """检索 → 拼资料 → LLM 生成带来源标注的答案。命中为空时不调用模型。"""
    hits = await search(db, query, top_k)
    if not hits:
