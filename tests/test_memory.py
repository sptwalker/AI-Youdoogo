...
from app.services import desktop_chat_service, memory_service
...
async def _seed_old_messages(db: AsyncSession) -> SysUser:
    ...
    for i in range(3):
        m = DesktopMessage(
            ...
            speaker_name="张三", content=f"消息{i}",
        )
        db.add(m)
        await db.flush()
...
async def test_archive_stores_distilled(
    db: AsyncSession, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    async def _fake_distill(_db: Any, transcript: str, **kw: Any) -> str:
        return "## 摘要\n提炼后的记忆内容"

    async def _fake_ingest(_db: Any, *, title: str, text: str, **kw: Any) -> Any:
        captured["title"] = title
        captured["text"] = text
        return None

    monkeypatch.setattr(memory_service, "distill_conversation", _fake_distill)
    monkeypatch.setattr(desktop_chat_service, "ingest_text", _fake_ingest)
    n = await desktop_chat_service.archive_old(db, user, days=10)
    assert n == 3
...
async def test_archive_falls_back_to_raw(
    db: AsyncSession, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    async def _fail_distill(_db: Any, transcript: str, **kw: Any) -> None:
        return None
    ...
    monkeypatch.setattr(memory_service, "distill_conversation", _fail_distill)
    monkeypatch.setattr(desktop_chat_service, "ingest_text", _fake_ingest)
    await desktop_chat_service.archive_old(db, user, days=10)
