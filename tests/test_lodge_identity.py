...
@pytest.mark.asyncio
async def test_lodge_user_target_token_is_verified_then_status_checked() -> None:
    key = _rsa_key()
    service, requests = _service(key)
...
@pytest.mark.asyncio
async def test_unknown_kid_refreshes_jwks_once_then_rejects() -> None:
    key = _rsa_key()
    service, requests = _service(key)
...
@pytest.mark.asyncio
async def test_status_mismatch_revocation_or_network_failure_is_denied() -> None:
    key = _rsa_key()
    statuses = [
    ...
    for status in statuses:
        service, _ = _service(key, status=status)
    ...
    timeout_service = LodgeIdentityService(
        ...
        status_url=STATUS_URL,
        status_service_token=STATUS_SERVICE_TOKEN,
        http_client=httpx.AsyncClient(transport=httpx.MockTransport(status_timeout)),
        now=lambda: NOW,
    )
    browser_token = _token(key)
...
@pytest.mark.asyncio
async def test_status_response_limit_and_cache_are_fail_closed_per_token_jti() -> None:
    key = _rsa_key()
...
@pytest.mark.asyncio
async def test_jwks_network_and_response_limit_fail_closed() -> None:
    key = _rsa_key()
    ...
    oversized = LodgeJwksClient(
        ...
        http_client=httpx.AsyncClient(
            transport=httpx.MockTransport(lambda _: httpx.Response(200, content=b"x" * 32))
        ),
