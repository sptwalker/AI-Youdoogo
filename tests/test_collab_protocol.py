...
async def test_execute_consult_runs_consulted_query(
    ...
    # 被咨询者答复里带取数指令 + 一条二次咨询（应被 exclude=collab 挡下）
    async def _run(_db: AsyncSession, role: AgentRole, **kw: Any) -> AgentTaskRecord:
        out = (
            "我去查一下。\n【取数】select count(*) c from v_event_4\n"
            "【咨询 @财务总监】顺带问下预算"  # 二次咨询，深度硬限应挡下
            if role.id == consultant.id
            else "盒子累计 1000 个。"  # 取数解读轮
        )
        return AgentTaskRecord(
            id=uuid.uuid4(), agent_role_id=role.id, task_type=kw.get("task_type", "t"),
    ...
    monkeypatch.setattr(query_skill, "run_agent", _run)
    monkeypatch.setattr(query_skill.data_query_service, "run_readonly_sql", _fake_sql)

    r = await proto.execute(db, initiator, "【咨询 @首席运营顾问】盒子累计数量是多少")
    # 被咨询者的取数被执行了（不再断链）
    assert executed.get("sql") == "select count(*) c from v_event_4"
    assert len(r.datasets) == 1 and r.datasets[0]["rows"] == [{"c": 1000}]
    # 二次咨询被挡下：consult_replies 只有 顾问本身 + 其取数解读，无"财务总监"
    assert not any(rec[0].name == "财务总监" for rec in r.consult_replies)
