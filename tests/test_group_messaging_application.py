...
async def test_subscription_releases_read_transaction_before_waiting() -> None:
    repository = FakeRepository()
    application, _, transactions = _application(repository, SequenceIdentifiers([]))

    events = [event async for event in application.subscribe_user_messages(uuid.UUID(int=1))]
