...
async def _mk(
    ...
    card = await svc.create_provider_record(
        db,
        CreateProviderCommand(
            name=name,
            tier=tier,
...
async def _rows(db: AsyncSession) -> list[dict[str, object]]:
    return [item.to_dict() for item in await svc.list_providers(db)]
...
async def test_first_card_of_tier_is_primary(db: AsyncSession) -> None:
    ...
    a = await _mk(db, "A")
    b = await _mk(db, "B")
    cards = {c["id"]: c for c in await _rows(db)}
...
async def test_seed_from_env_creates_two_cards(
    ...
    n = await svc.seed_providers_from_environment(db)
    assert n == 2
    cards = await _rows(db)
...
async def test_seed_from_env_idempotent(
    ...
    monkeypatch.setattr("app.core.config.get_settings", lambda: _S())
    await _mk(db, "已有卡")
    assert await svc.seed_providers_from_environment(db) == 0
...
async def test_set_primary_exclusive_within_tier(db: AsyncSession) -> None:
    ...
    await svc.set_primary_record(db, b)
    cards = {c["id"]: c for c in await _rows(db)}
...
async def test_disable_primary_promotes_sibling(db: AsyncSession) -> None:
    ...
    a = await _mk(db, "A", "daily")
    b = await _mk(db, "B", "daily")
    await svc.toggle_active_record(db, a, False)  # a 是主用
    cards = {c["id"]: c for c in await _rows(db)}
...
async def test_delete_primary_promotes_sibling(db: AsyncSession) -> None:
    ...
    a = await _mk(db, "A", "daily")
    b = await _mk(db, "B", "daily")
    await svc.delete_provider_record(db, a)
    rows = await _rows(db)
...
async def test_sync_to_factory_registers_active_and_status(db: AsyncSession) -> None:
    ...
    b = await _mk(db, "B", "daily")
    await svc.set_primary_record(db, b)
    await svc.toggle_active_record(db, a, False)
    await svc.synchronize_provider_runtime(db)
...
async def test_get_llm_for_role_no_card_raises(db: AsyncSession) -> None:
    ...
    await svc.synchronize_provider_runtime(db)  # 无卡片
...
async def test_test_provider_persists_status(
    ...
    a = await _mk(db, "A")
    result = await svc.test_provider_record(db, a, tester=ModelCardTester(_fake))
...
async def test_invalid_tier_rejected(db: AsyncSession) -> None:
    ...
    with pytest.raises(ApplicationError, match="tier"):
        await svc.create_provider_record(
            db,
            CreateProviderCommand(
                name="X", tier="bogus", base_url="u", api_key="k", model="m"
            ),
        )
