...
async def test_list_events_merges_alias_and_isolates_failure(
    ...
    groups = [snapshot.to_dict() for snapshot in await analytics.list_events(db, date(2026, 7, 15))]
...
async def test_save_aliases_upsert_and_clear(db: AsyncSession) -> None:
    ...
    n1 = await analytics.save_event_aliases(
        db,
        (EventAliasUpdate("v_event_4", "new_device", "设备激活"),),
    )
    assert n1 == 1
    # 改名（同键 upsert，不新增行）
    await analytics.save_event_aliases(
        db,
        (EventAliasUpdate("v_event_4", "new_device", "激活设备"),),
    )
    alive = (
    ...
    await analytics.save_event_aliases(
        db,
        (EventAliasUpdate("v_event_4", "new_device", ""),),
    )
    alive2 = (
        (await db.execute(select(TdEventAlias).where(TdEventAlias.is_delete.is_(False))))
        .scalars()
...
async def test_save_aliases_skips_invalid(db: AsyncSession) -> None:
    ...
    n = await analytics.save_event_aliases(
        db,
        (
            EventAliasUpdate("", "x", "a"),
            EventAliasUpdate("v", "", "b"),
        ),
    )
    assert n == 0
